...
export default function RecordList() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await fetch(
        `https://sheltered-depths-34196.herokuapp.com/project/${params.id.toString()}`
      );
  ...
  useEffect(() => {
    async function getRecords() {
      const response = await fetch(
        `https://sheltered-depths-34196.herokuapp.com/record/`
      );
  ...
  async function deleteRecord(id) {
    await fetch(`https://sheltered-depths-34196.herokuapp.com/${id}`, {
      method: "DELETE",
    });
  ...
  async function updateRecord(updatedTask) {
    await fetch(
      `https://sheltered-depths-34196.herokuapp.com/update/${updatedTask._id}`,
      {
        method: "POST",
      ...
      return (
        ...
        >
          ...
          <div id="container2">
            {records.map((record) => {
              if (
                record.status === myStatus &&
                myProject._id === record.project
              ) {
                return (
                  <div
                    style={{
                      backgroundColor: isDarkMode ? "#838383" : "white",
                      color: isDarkMode ? "white" : "#1f1f1f",
                    }}
                    key={record._id}
                    onClick={() => {
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              display: showCreate === "block" ? "none" : "flex",
            }}
          >
            <button
              className="btn btn-danger"
              onClick={() => updateRecord(myTask)}
            >
              Save
            </button>
            <button
          ...
          <button
            className="btn btn-danger"
            style={{ width: "100%", display: showCreate }}
  ...
  async function createTask(e) {
    ...
    await fetch("https://sheltered-depths-34196.herokuapp.com/record/add", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  return (
    <>
      <h3
        style={{ marginLeft: "6.5%", color: isDarkMode ? "white" : "#1f1f1f" }}
      >
        {myProject.name}
      </h3>
      <div id="container">{todoListBuckets()}</div>
      {editForm()}
    </>
  );
}
